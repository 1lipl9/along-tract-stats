...
fit_trk_model2 <- function(df){
    lme.trk = tryCatch(lme(FA ~ Point/Group - 1, 
...
sigF.trk_data   = merge(trk_data, data.frame(Tract=sigF.Tract, 
...
models$tTable = ddply(models$tTable, c("Tract", "Hemisphere"),
                      transform, Position = (as.numeric(Point)-1) * 
...
p1 <- p1 + geom_histogram(binwidth=0.1) + geom_density(color='blue') + 
  geom_vline(xintercept = sort(maxT.no.na)[crit], color='red') + xlim(0,5) + 
  ylim(0,1.75) + 
  labs(x=expression(paste('Empirical max ', group('|', italic(t), '|'))), y='Density')

########
# Plot # of streamlines by Group, tract, and hemisphere
p2 = ggplot(trk_props_long, aes(x=Group, y=Streamlines, fill=Group)) + 
  geom_bar(stat='summary', fun.y=mean) + facet_grid(Tract~Hemisphere)
...
get_bars <- function(y.in){
    data.frame(y=mean(y.in), ymax=mean(y.in)+1.96*stderr(y.in), 
               ymin=mean(y.in)-1.96*stderr(y.in))
}
...
# Make final plot
p2 <- p2 + set_coords + brew_fill + sub_pts + error_bars + set_opts


########
# Plot FA vs. position, conditioned on hemisphere, tract, and group
p3 = qplot(Position, FA, group=ID, colour=Group, 
           size=Streamlines, alpha=I(0.3), data=trk_data, facets = Tract~Hemisphere, 
...
# Add group means
means_smooth = stat_smooth(aes(ymax=..y..+1.96*..se.., ymin=..y..-1.96*..se.., 
                               group=Group), alpha=0.8, span=0.5)
...
grp_effect = geom_text(aes(x=0, y=0.2, label=Caption, group=NULL, size=NULL), 
                       data=anova_grp, colour='black')
...
get_breaks <- function(df, thresh=0.05){
    ...
    dsig = c(diff(sig), 0)
    if(subset(models$anova, Term=="Point:Group" & Tract==df$Tract[1] & 
              Hemisphere==df$Hemisphere[1])$p.value < thresh){
...
sig_bars   = geom_segment(aes(x=on, y=0.2, xend=off, yend=0.2, group=NULL, size=NULL), 
                          data=break_list, colour='black')

# Make final plot
p3 <- p3 + brew_cols + means_smooth + grp_effect + sig_bars
...
models$tTable.interp = ddply(models$tTable, c("Tract", "Hemisphere"), 
                             colwise(lin_interp, c('Point','t.value', 'p.value', 
                                                   'Position')))

p4 = ggplot(data=models$tTable.interp, aes(x=Position, y=p.value, color=p.value < 0.05, 
                                           group=1)) + geom_line() + 
  facet_grid(facets=Tract~Hemisphere) + xlab('Position along tract (%)') + 
  scale_color_manual('p.value < 0.05', values=c('red', 'green'))

# Gray out non-significant areas
grayout_p = annotate('rect', xmin=0, xmax=100, ymin=0.05, ymax=1, alpha=0.25)

# Make final plot
p5 <- p4 + grayout_p + scale_y_log10(limits=c(0.001, 1), breaks=c(0.001, 0.01, 0.1, 1))

########
# Plot t-statistics
# Gray out non-significant areas
tcrits = ddply(models$tTable, c("Tract", "Hemisphere"), summarize, t.crit=qt(0.05/2, 
                                                                             DF[1]))
grayout_t = geom_rect(aes(x=1, y=1, ymin=t.crit, ymax=-t.crit), color=NA, xmin=0, 
                      xmax=100, alpha=0.25, data=tcrits)

# Make final plot
p6 <- p4 + grayout_t + aes(y=t.value, 
                           color=t.value < tcrits$t.crit | t.value > -tcrits$t.crit) + 
  ylim(c(-4, 4)) + geom_hline(yintercept = 0, linetype=3)

################################################################################
# Output statistical results for import into MATLAB and overlay onto mean tract 
# geometry
write.table(models$tTable, file=file.path(exptDir, 'effects_table.txt'), quote=F, 
            row.names=F)
